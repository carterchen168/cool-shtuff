...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        other = expressions.rest.first
        evaluation = scheme_eval(other, env)
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        display = signature.first
        rest = signature.rest
        exp_rest = expressions.rest
        while rest is not nil:
            if not scheme_symbolp(rest.first):
                raise SchemeError
            rest = rest.rest
        env.define(display, LambdaProcedure(signature.rest, exp_rest, env))
        return display
        # END PROBLEM 10
    else:
